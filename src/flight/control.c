...
//    }
  ...
  if ((flags.arm_state == 0) || flags.failsafe || (state.throttle < 0.001f)) {
    // CONDITION: disarmed OR failsafe OR throttle off

    if (onground_long && (gettime() - onground_long > ON_GROUND_LONG_TIMEOUT)) {
      onground_long = 0;
    }
    ...
    if (profile_motor.throttle_boost > 0.0f) {
      state.throttle += (float)(profile_motor.throttle_boost) * throttlehpf(state.throttle);
      if (state.throttle < 0)
        state.throttle = 0;
